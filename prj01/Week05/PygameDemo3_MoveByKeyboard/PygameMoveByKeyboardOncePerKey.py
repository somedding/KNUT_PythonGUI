...
bombImage = pygame.image.load('images/5846275.png')

ballX = random.randrange(MAX_WIDTH)
ballY = random.randrange(MAX_HEIGHT)
targetRect = pygame.Rect(TARGET_X, TARGET_Y, TARGET_WIDTH_HEIGHT, TARGET_WIDTH_HEIGHT)
bombRect = pygame.Rect(BOMB_X, BOMB_Y, BOMB_WIDTH_HEIGHT,BOMB_WIDTH_HEIGHT)
...
while True:
    ...
    bombRect = pygame.Rect(ballX, ballY, BALL_WIDTH_HEIGHT, BALL_WIDTH_HEIGHT)
    if ballRect.colliderect(targetRect):
        print('You stepped on a bomb')
    window.fill(BLACK)
    window.blit(targetImage, (TARGET_X, TARGET_Y))
    window.blit(ballImage, (ballX, ballY))
    window.blit(bombImage, (BOMB_X, BOMB_Y))
